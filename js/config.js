...
function formatTitleText(value) {
    ...
    return clean
        .split(/\s*·\s*/)
        .map(part => toTitleCaseEs(part.trim()))
        .join(' · ');
}
...
function normalizeLayoutForTheme(layout = {}) {
    ...
    next.title = {
        ...titleObj,
        text: titleText ? `<b>${escapeHtml(titleText)}</b>` : '',
        font: {
            ...(titleObj.font || {}),
    ...
    next.legend = {
        ...
        bgcolor: normalizeColorForTheme(next.legend?.bgcolor || (C.isLight ? 'rgba(255,255,255,0.96)' : 'rgba(12,12,12,0.78)')),
        bordercolor: normalizeColorForTheme(next.legend?.bordercolor || C.borderColor),
...
if (window.Plotly && !window.__metrixPlotlyPatched) {
    ...
    window.Plotly.newPlot = function (gd, data, layout, config) {
        ...
        if (Array.isArray(safeData)) {
            ...
            if (filterableCount >= 2 && safeLayout.showlegend !== false) {
